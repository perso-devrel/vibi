...
/**
 * Unit-level verification that [DubCastDatabase.MIGRATION_7_8_STATEMENTS] rewrites
 * the schema and preserves data as described in the Task 3a plan. Runs against an
 * in-memory SQLite via sqlite-jdbc so no Android runtime / MigrationTestHelper is
 * required.
 */
class Migration7To8Test {
    ...
    private fun applyMigration() {
        ...
        try {
            for (sql in DubCastDatabase.MIGRATION_7_8_STATEMENTS) {
    ...
    private fun seedV7Data() {
        conn.createStatement().use { stmt ->
            ...
            // Two child rows per table per project
